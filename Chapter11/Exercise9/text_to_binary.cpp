/*
   Chapter 11 Exercise 9 Part 1, Page 409
   Program that converts an ordinary text file to binary
...
try {
   ...
   const string output_filename = change_file_extension(argv[1], "bin");
   ofstream out;
   open_file_output(output_filename, out, true);

   for (string s; getline(in, s);) {
      s += '\n';
      out.write(as_bytes(s), s.size());
   }
